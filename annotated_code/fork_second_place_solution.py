...
cores = 4 # num of cpu cores
max_text_length=60################### fixed length for all words
min_df_one=5
min_df_bi=5

def clean_str(text):
    ''' lower all texts. replace pronouced characters with letter. replace non-letters and non-numbers with space. remove extra spaces.
        if missing then NaN
    '''
    try:
...
def load_data( ):
    ''' load tsv. fill categorical with mode. 
        split: up threshold to ensure training data size.
        only consider positive price. log transformed price and set to float32 accuracy.
    '''
...
def create_count_features(df_data):
    ''' add a feature about number of words in item description
    '''
    def lg(text):
...
def parallelize_dataframe(df, func):
    ''' split dataframe based number of cpu cores. initiated pools for parallel processing, apply functions within pool and 
    ...
    return df
def clean_str_df(df):
    ''' for each of the field in the dataframe, try applying clean_str function
    '''
    return df.apply( lambda s : clean_str(s))

def prepare_data(df_data, train=True):
    ''' main function to apply all the helpers sequentially
    '''
    print ('Prepare data....')
    
    def fill_brand_name(x):
        '''??try different (1 to 4) ngrams splitting param,  
        '''
        try:
            k=[]
            for n in [4,3,2,1]:
    ...
    def fill_cat(x, i, new=False): # never used
        ''' if new, return only one stripped string of the specified loc;
            if not, return the full string with backslash removed and stripped
        '''
        try:
            if new:
    ...
    # create count feature for item desc, and convert it to pct based on max_text_length
    create_count_features(df_data)     
    df_data['nb_words_item_description'] /= max_text_length

    # apply clean_str_df to brand_name, name and item_description
    ...
    # look at brandname from the item desc.
    ...
    # long feat concat all the brand categorical features
...
# # lemmatization
# Lemmatization is the process of converting a word to its base form within given context
...
def create_bigrams(text):
    try:
        ...
        lst_bi = []
        for combo in combinations(text, 2): # all possible combinations, with both orders
            cb1=combo[0]+combo[1] 
            cb2=combo[1]+combo[0]
            in_dict=False

            # check if one form of the combo is a word already in the dictionary
            # if it does, add it to the list. its possible to add both orders
            # if not, create a word with ___, to indicate its not a natrual word, and add it to the list
...
brand_names = df_train.groupby('brand_name').size()  # series with brand_name as index
...
for feat in ['name','item_description' ]:
    df_train[feat].apply(             lambda x : word_count(x, word_count_dict_one) ) # count occurance of each word in name and item_description 
rare_words = [key for key in word_count_dict_one if  word_count_dict_one[key]<min_df_one ] # manual threshold count of 5. list of rare words.
for key in rare_words :
    word_count_dict_one.pop(key, None) # remove rare words from the dict
for feat in ['name','item_description' ]:
    df_train[feat]      = df_train[feat].apply( lambda x : remove_low_freq(x, word_count_dict_one) ) # return string without low freq words
word_count_dict_one=dict(word_count_dict_one) # convert word_count_dict_one 
...
df_train['name_bi']      = parallelize_dataframe( df_train['name_desc'],  create_bigrams_df ) # output list of word combos
df_train['name_bi'].apply(word_count_bi ) # count occurance of word combos
rare_words = [key for key in word_count_dict_bi if  word_count_dict_bi[key]<min_df_bi ]
for key in rare_words :
    word_count_dict_bi.pop(key, None)
df_train['name_bi']      = df_train['name_bi'].apply( lambda x : remove_low_freq(x, word_count_dict_bi) ) # remove rare bigrams
print('[{}] Finished CREATING BIGRAMS...'.format(time.time() - start_time))

#####################################

# merge bigram and word as vocabulary dict
# ?? need to check cpt assignment. if in either of the vocabulary then 0, in both then 1. The latter condition might never be true
...
# encode all short categorical variables based on mean encoding on price
# normalize them based on max price
# fill na when the price of the categorical value is missing, with mean price of all categorical values. 
# ?? its plain average, not considering occurance weights
...
# convert name and item_description to matrix based on word and bigram
...
# all categorical fields aleady transformed
# name and item_desc with word freq counts
# brand_name, category_name, category1, category2, category3 etc with LabelEncoder
...
#FASTTEXT MODEL
# FastText improves on Word2Vec by taking word parts into account, too. 
...
def fasttext_model():
    ...
    # https://stackoverflow.com/questions/42122168/keras-how-to-construct-a-shared-embedding-layer-for-each-input-neuron/42123881
...
def sparse_char_model():                                             
    ...
    # PReLU: similar to leaky ReLU, slope alpha is learned in the training process
...
submission_idx = []
chunk_counter=1
for df_submission in pd.read_csv(path+'test_stg2.tsv', sep='\t', encoding='utf-8' , chunksize=800000):
...
# prediction essemble: mean vs linear
